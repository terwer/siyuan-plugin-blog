# 核心功能

<cite>
**本文引用的文件**
- [apps/siyuan/src/pages/Share.vue](file://apps/siyuan/src/pages/Share.vue)
- [apps/siyuan/src/stores/useSettingStore.ts](file://apps/siyuan/src/stores/useSettingStore.ts)
- [apps/siyuan/src/composables/useStaticShare.ts](file://apps/siyuan/src/composables/useStaticShare.ts)
- [apps/siyuan/src/composables/useSiyuanApi.ts](file://apps/siyuan/src/composables/useSiyuanApi.ts)
- [apps/siyuan/src/pages/Setting/GeneralSetting.vue](file://apps/siyuan/src/pages/Setting/GeneralSetting.vue)
- [apps/siyuan/src/pages/Setting/AdvancedSetting.vue](file://apps/siyuan/src/pages/Setting/AdvancedSetting.vue)
- [apps/app/pages/index.vue](file://apps/app/pages/index.vue)
- [apps/app/composables/useCommonShareType.ts](file://apps/app/composables/useCommonShareType.ts)
- [apps/app/enums/ShareTypeEnum.ts](file://apps/app/enums/ShareTypeEnum.ts)
- [apps/app/composables/useClientThemeMode.ts](file://apps/app/composables/useClientThemeMode.ts)
- [apps/app/utils/ThemeUtils.ts](file://apps/app/utils/ThemeUtils.ts)
- [apps/siyuan/src/i18n/zh_CN.json](file://apps/siyuan/src/i18n/zh_CN.json)
- [apps/app/i18n/locales/zh_CN.json](file://apps/app/i18n/locales/zh_CN.json)
- [apps/app/composables/useViewerCapabilities.ts](file://apps/app/composables/useViewerCapabilities.ts)
- [apps/app/composables/useSiyuanSPA.ts](file://apps/app/composables/useSiyuanSPA.ts)
- [apps/app/composables/useAuthModeFetch.ts](file://apps/app/composables/useAuthModeFetch.ts)
- [apps/app/components/static/HomePage.vue](file://apps/app/components/static/HomePage.vue)
- [apps/app/components/static/DetailPage.vue](file://apps/app/components/static/DetailPage.vue)
- [apps/siyuan/src/stores/common/useCommonStorageAsync.ts](file://apps/siyuan/src/stores/common/useCommonStorageAsync.ts)
- [apps/siyuan/src/composables/useDataTable.ts](file://apps/siyuan/src/composables/useDataTable.ts)
- [DEVELOPMENT.md](file://DEVELOPMENT.md)
- [apps/app/script/siyuan.sh](file://apps/app/script/siyuan.sh)
</cite>

## 更新摘要
**所做更改**
- 新增 SPA 数据获取系统的延迟加载和错误处理改进章节
- 新增 viewerCapabilities 组合式钩子的引入与使用说明
- 更新了 AI 助手功能的架构说明和部署规则
- 增强了静态页面加载性能和用户体验的描述

## 目录
1. [引言](#引言)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [SPA 数据获取系统增强](#spa-数据获取系统增强)
7. [AI 助手与 viewerCapabilities](#ai-助手与-viewercapabilities)
8. [依赖关系分析](#依赖关系分析)
9. [性能考量](#性能考量)
10. [故障排查指南](#故障排查指南)
11. [结论](#结论)
12. [附录](#附录)

## 引言
本文件面向 Siyuan Plugin Blog 的使用者与维护者，系统性阐述"内容分享系统"的工作原理与实现细节，覆盖以下关键主题：
- 如何将 Siyuan 笔记转换为可分享的静态网页链接
- 分享类型与权限控制（公开分享、私有静态分享、主页设置、过期时间）
- 设置管理（基础设置、高级配置、多语言支持）
- 主题系统（主题选择、动态加载、深浅色模式）
- SPA 数据获取系统的延迟加载和错误处理改进
- viewerCapabilities 组合式钩子的引入与使用
- AI 助手功能的架构与部署规则
- 实际使用示例与最佳实践

## 项目结构
本项目采用前后端分离与多应用布局：
- Siyuan 插件前端（Vue 组合式 API + Pinia Store）：负责分享开关、链接生成、设置管理、静态资源落盘与清理
- Web 应用前端（Nuxt/Vue）：负责展示静态分享页面、主题加载、权限判断、页面渲染和数据获取

```mermaid
graph TB
subgraph "Siyuan 插件前端"
SharePage["分享页<br/>Share.vue"]
SettingStore["设置存储<br/>useSettingStore.ts"]
StaticShare["静态分享处理<br/>useStaticShare.ts"]
SiyuanAPI["Siyuan API 封装<br/>useSiyuanApi.ts"]
GenSetting["基础设置页<br/>GeneralSetting.vue"]
AdvSetting["高级设置页<br/>AdvancedSetting.vue"]
CommonStorage["通用存储<br/>useCommonStorageAsync.ts"]
DataTable["数据表格<br/>useDataTable.ts"]
end
subgraph "Web 应用前端"
IndexPage["首页路由<br/>index.vue"]
ShareType["分享类型判断<br/>useCommonShareType.ts"]
ShareEnum["分享类型枚举<br/>ShareTypeEnum.ts"]
ThemeMode["主题模式与加载<br/>useClientThemeMode.ts"]
ThemeUtils["主题路径工具<br/>ThemeUtils.ts"]
ViewerCaps["查看器能力<br/>useViewerCapabilities.ts"]
SiyuanSPA["Siyuan SPA 检测<br/>useSiyuanSPA.ts"]
AuthFetch["认证模式获取<br/>useAuthModeFetch.ts"]
HomePage["静态首页<br/>HomePage.vue"]
DetailPage["详情页面<br/>DetailPage.vue"]
end
SharePage --> SiyuanAPI
SharePage --> StaticShare
SharePage --> SettingStore
GenSetting --> SettingStore
AdvSetting --> SettingStore
CommonStorage --> DataTable
IndexPage --> ShareType
ShareType --> ShareEnum
IndexPage --> ThemeMode
ThemeMode --> ThemeUtils
IndexPage --> ViewerCaps
ViewerCaps --> SiyuanSPA
IndexPage --> AuthFetch
AuthFetch --> HomePage
AuthFetch --> DetailPage
```

**图表来源**
- [apps/siyuan/src/pages/Share.vue:10-244](file://apps/siyuan/src/pages/Share.vue#L10-L244)
- [apps/siyuan/src/stores/useSettingStore.ts:20-79](file://apps/siyuan/src/stores/useSettingStore.ts#L20-L79)
- [apps/siyuan/src/composables/useStaticShare.ts:17-96](file://apps/siyuan/src/composables/useStaticShare.ts#L17-L96)
- [apps/siyuan/src/composables/useSiyuanApi.ts:17-29](file://apps/siyuan/src/composables/useSiyuanApi.ts#L17-L29)
- [apps/siyuan/src/pages/Setting/GeneralSetting.vue:95-118](file://apps/siyuan/src/pages/Setting/GeneralSetting.vue#L95-L118)
- [apps/siyuan/src/pages/Setting/AdvancedSetting.vue:36-65](file://apps/siyuan/src/pages/Setting/AdvancedSetting.vue#L36-L65)
- [apps/siyuan/src/stores/common/useCommonStorageAsync.ts:21-63](file://apps/siyuan/src/stores/common/useCommonStorageAsync.ts#L21-L63)
- [apps/siyuan/src/composables/useDataTable.ts:66-96](file://apps/siyuan/src/composables/useDataTable.ts#L66-L96)
- [apps/app/pages/index.vue:10-28](file://apps/app/pages/index.vue#L10-L28)
- [apps/app/composables/useCommonShareType.ts:12-53](file://apps/app/composables/useCommonShareType.ts#L12-L53)
- [apps/app/enums/ShareTypeEnum.ts:13-25](file://apps/app/enums/ShareTypeEnum.ts#L13-L25)
- [apps/app/composables/useClientThemeMode.ts:23-97](file://apps/app/composables/useClientThemeMode.ts#L23-L97)
- [apps/app/utils/ThemeUtils.ts:18-35](file://apps/app/utils/ThemeUtils.ts#L18-L35)
- [apps/app/composables/useViewerCapabilities.ts:1-15](file://apps/app/composables/useViewerCapabilities.ts#L1-L15)
- [apps/app/composables/useSiyuanSPA.ts:1-16](file://apps/app/composables/useSiyuanSPA.ts#L1-L16)
- [apps/app/composables/useAuthModeFetch.ts:15-324](file://apps/app/composables/useAuthModeFetch.ts#L15-L324)
- [apps/app/components/static/HomePage.vue:1-75](file://apps/app/components/static/HomePage.vue#L1-L75)
- [apps/app/components/static/DetailPage.vue:1-21](file://apps/app/components/static/DetailPage.vue#L1-L21)

**章节来源**
- [apps/siyuan/src/pages/Share.vue:10-244](file://apps/siyuan/src/pages/Share.vue#L10-L244)
- [apps/app/pages/index.vue:10-28](file://apps/app/pages/index.vue#L10-L28)

## 核心组件
- 分享页组件：负责分享开关、链接生成、IP 切换、过期时间设置、主页设置、一键清空所有分享
- 设置存储：集中读取/更新全局配置（站点 URL、主页 ID、主题、分享模板等），并可注入自定义 CSS
- 静态分享处理：将文章内容与资源落盘至公共目录，生成 JSON 与资产目录；支持更新与清理
- 设置页组件：基础设置（站点 URL、主页 ID、主题）与高级设置（站点标题/副标题/描述、底部信息、分享模板）
- 分享类型判断：统一返回"私有静态分享"类型，确保安全与一致性
- 主题模式与加载：根据深浅色模式与主题参数动态加载主题 CSS 与高亮样式，并支持自定义 CSS 注入
- **新增** SPA 数据获取系统：支持延迟加载、错误处理和多种获取模式
- **新增** viewerCapabilities 组合式钩子：检测查看器能力和 AI 助手支持状态

**章节来源**
- [apps/siyuan/src/pages/Share.vue:30-244](file://apps/siyuan/src/pages/Share.vue#L30-L244)
- [apps/siyuan/src/stores/useSettingStore.ts:20-79](file://apps/siyuan/src/stores/useSettingStore.ts#L20-L79)
- [apps/siyuan/src/composables/useStaticShare.ts:17-96](file://apps/siyuan/src/composables/useStaticShare.ts#L17-L96)
- [apps/siyuan/src/pages/Setting/GeneralSetting.vue:95-118](file://apps/siyuan/src/pages/Setting/GeneralSetting.vue#L95-L118)
- [apps/siyuan/src/pages/Setting/AdvancedSetting.vue:36-65](file://apps/siyuan/src/pages/Setting/AdvancedSetting.vue#L36-L65)
- [apps/app/composables/useCommonShareType.ts:12-53](file://apps/app/composables/useCommonShareType.ts#L12-L53)
- [apps/app/composables/useClientThemeMode.ts:23-97](file://apps/app/composables/useClientThemeMode.ts#L23-L97)
- [apps/app/composables/useViewerCapabilities.ts:1-15](file://apps/app/composables/useViewerCapabilities.ts#L1-L15)
- [apps/app/composables/useSiyuanSPA.ts:1-16](file://apps/app/composables/useSiyuanSPA.ts#L1-L16)
- [apps/app/composables/useAuthModeFetch.ts:15-324](file://apps/app/composables/useAuthModeFetch.ts#L15-L324)

## 架构总览
下图展示了从"笔记分享"到"静态网页访问"的端到端流程，包括新增的 SPA 数据获取系统和 viewerCapabilities 能力检测。

```mermaid
sequenceDiagram
participant User as "用户"
participant SiyuanUI as "Siyuan 分享页<br/>Share.vue"
participant Store as "设置存储<br/>useSettingStore.ts"
participant Static as "静态分享处理<br/>useStaticShare.ts"
participant Kernel as "Siyuan 内核接口<br/>useSiyuanApi.ts"
participant WebApp as "Web 应用首页<br/>index.vue"
participant Theme as "主题加载<br/>useClientThemeMode.ts"
participant ViewerCaps as "查看器能力<br/>useViewerCapabilities.ts"
User->>SiyuanUI : 开启"在线分享"
SiyuanUI->>Kernel : 设置块属性发布状态/时间
SiyuanUI->>Static : openStaticShare(文档ID, 文章内容)
Static->>Kernel : 保存JSON与下载资源到公共目录
Static-->>SiyuanUI : 成功
SiyuanUI-->>User : 显示分享链接
User->>WebApp : 打开分享链接
WebApp->>ViewerCaps : 检测查看器能力
ViewerCaps-->>WebApp : 返回能力支持状态
WebApp->>Theme : 动态加载主题CSS与高亮样式
Theme-->>WebApp : 完成主题渲染
WebApp-->>User : 展示静态分享页面
```

**图表来源**
- [apps/siyuan/src/pages/Share.vue:66-119](file://apps/siyuan/src/pages/Share.vue#L66-L119)
- [apps/siyuan/src/composables/useStaticShare.ts:62-83](file://apps/siyuan/src/composables/useStaticShare.ts#L62-L83)
- [apps/siyuan/src/composables/useSiyuanApi.ts:17-29](file://apps/siyuan/src/composables/useSiyuanApi.ts#L17-L29)
- [apps/app/pages/index.vue:17-22](file://apps/app/pages/index.vue#L17-L22)
- [apps/app/composables/useClientThemeMode.ts:57-97](file://apps/app/composables/useClientThemeMode.ts#L57-L97)
- [apps/app/composables/useViewerCapabilities.ts:1-15](file://apps/app/composables/useViewerCapabilities.ts#L1-L15)

## 详细组件分析

### 内容分享系统（从笔记到静态链接）
- 分享开关与属性写入：开启分享时写入发布状态与发布时间；关闭分享时重置状态并删除静态资源
- 静态资源落盘：将文章内容与资源下载到公共目录，仅暴露必要字段并持久化 JSON
- 链接生成与复制：支持模板化复制（支持占位符替换），并可切换 IP 以适配不同网络环境
- 过期时间与主页设置：支持设置链接有效期（秒，上限 7 天），支持设置主页 ID；提供一键清空所有分享

```mermaid
flowchart TD
Start(["开始"]) --> Toggle["点击"在线分享""]
Toggle --> IsOn{"是否开启分享？"}
IsOn --> |是| SetAttrs["写入发布状态与时间"]
SetAttrs --> OpenStatic["openStaticShare(文档ID, 文章内容)"]
OpenStatic --> SaveJSON["保存JSON到公共目录"]
SaveJSON --> DownloadAssets["下载资源到公共目录"]
DownloadAssets --> Done["完成"]
IsOn --> |否| ResetAttrs["重置发布状态与时间"]
ResetAttrs --> CloseStatic["closeStaticShare(文档ID)"]
CloseStatic --> RemoveFiles["删除JSON与资源目录"]
RemoveFiles --> Done
Done --> End(["结束"])
```

**图表来源**
- [apps/siyuan/src/pages/Share.vue:66-119](file://apps/siyuan/src/pages/Share.vue#L66-L119)
- [apps/siyuan/src/composables/useStaticShare.ts:62-83](file://apps/siyuan/src/composables/useStaticShare.ts#L62-L83)

**章节来源**
- [apps/siyuan/src/pages/Share.vue:66-244](file://apps/siyuan/src/pages/Share.vue#L66-L244)
- [apps/siyuan/src/composables/useStaticShare.ts:17-96](file://apps/siyuan/src/composables/useStaticShare.ts#L17-L96)

### 分享类型与权限控制
- 分享类型：统一返回"私有静态分享"，不再支持公共分享模式，确保安全性
- 权限控制：通过"发布状态"与"过期时间"共同决定访问权限；主页设置用于确定入口页面
- 链接模板：支持占位符替换，便于复制分享文案

```mermaid
classDiagram
class ShareTypeEnum {
+ShareType_Public
+ShareType_Static
}
class useCommonShareType {
+getShareType() ShareTypeEnum
+isPrivateShare() Promise~boolean~
}
useCommonShareType --> ShareTypeEnum : "返回类型"
```

**图表来源**
- [apps/app/enums/ShareTypeEnum.ts:13-25](file://apps/app/enums/ShareTypeEnum.ts#L13-L25)
- [apps/app/composables/useCommonShareType.ts:12-53](file://apps/app/composables/useCommonShareType.ts#L12-L53)

**章节来源**
- [apps/app/enums/ShareTypeEnum.ts:13-25](file://apps/app/enums/ShareTypeEnum.ts#L13-L25)
- [apps/app/composables/useCommonShareType.ts:12-53](file://apps/app/composables/useCommonShareType.ts#L12-L53)

### 设置管理（基础设置、高级配置、多语言）
- 基础设置：站点 URL、主页 ID、浅色/深色主题选择；保存后更新全局配置
- 高级设置：站点标题/副标题/描述、底部信息、分享模板；支持 HTML 与占位符
- 多语言：插件前端与 Web 前端分别提供中文语言包，覆盖分享、设置、主题等模块

```mermaid
sequenceDiagram
participant User as "用户"
participant Gen as "基础设置页<br/>GeneralSetting.vue"
participant Adv as "高级设置页<br/>AdvancedSetting.vue"
participant Store as "设置存储<br/>useSettingStore.ts"
User->>Gen : 修改站点URL/主页ID/主题
Gen->>Store : updateSetting(合并后的配置)
Store-->>Gen : 保存成功
User->>Adv : 修改站点标题/描述/底部/模板
Adv->>Store : updateSetting(合并后的配置)
Store-->>Adv : 保存成功
```

**图表来源**
- [apps/siyuan/src/pages/Setting/GeneralSetting.vue:104-118](file://apps/siyuan/src/pages/Setting/GeneralSetting.vue#L104-L118)
- [apps/siyuan/src/pages/Setting/AdvancedSetting.vue:52-65](file://apps/siyuan/src/pages/Setting/AdvancedSetting.vue#L52-L65)
- [apps/siyuan/src/stores/useSettingStore.ts:50-62](file://apps/siyuan/src/stores/useSettingStore.ts#L50-L62)

**章节来源**
- [apps/siyuan/src/pages/Setting/GeneralSetting.vue:95-118](file://apps/siyuan/src/pages/Setting/GeneralSetting.vue#L95-L118)
- [apps/siyuan/src/pages/Setting/AdvancedSetting.vue:36-65](file://apps/siyuan/src/pages/Setting/AdvancedSetting.vue#L36-L65)
- [apps/siyuan/src/stores/useSettingStore.ts:20-79](file://apps/siyuan/src/stores/useSettingStore.ts#L20-L79)
- [apps/siyuan/src/i18n/zh_CN.json:1-53](file://apps/siyuan/src/i18n/zh_CN.json#L1-L53)
- [apps/app/i18n/locales/zh_CN.json:1-100](file://apps/app/i18n/locales/zh_CN.json#L1-L100)

### 主题系统（架构与使用）
- 主题加载：根据深浅色模式与主题参数动态加载默认主题与当前主题 CSS，以及高亮样式
- 自定义 CSS：从内核接口拉取启用的 CSS 片段并注入到页面
- 路径工具：提供带 base 的路径拼接能力，确保资源引用正确

```mermaid
sequenceDiagram
participant Page as "Web 应用首页<br/>index.vue"
participant Mode as "主题模式与加载<br/>useClientThemeMode.ts"
participant Utils as "主题路径工具<br/>ThemeUtils.ts"
Page->>Mode : 初始化主题深浅色、主题名、版本
Mode->>Mode : useHead 注入默认主题与当前主题CSS
Mode->>Mode : 注入高亮样式与自定义CSS片段
Mode->>Utils : withBase(拼接base路径)
Utils-->>Mode : 返回带base的路径
Mode-->>Page : 完成主题渲染
```

**图表来源**
- [apps/app/pages/index.vue:17-22](file://apps/app/pages/index.vue#L17-L22)
- [apps/app/composables/useClientThemeMode.ts:57-97](file://apps/app/composables/useClientThemeMode.ts#L57-L97)
- [apps/app/utils/ThemeUtils.ts:22-34](file://apps/app/utils/ThemeUtils.ts#L22-L34)

**章节来源**
- [apps/app/composables/useClientThemeMode.ts:23-157](file://apps/app/composables/useClientThemeMode.ts#L23-L157)
- [apps/app/utils/ThemeUtils.ts:18-35](file://apps/app/utils/ThemeUtils.ts#L18-L35)

## SPA 数据获取系统增强

### 延迟加载机制
SPA 数据获取系统引入了智能的延迟加载机制，通过 `useAuthModeFetch` 组合式钩子实现：

- **按需加载**：只有在实际需要时才发起数据请求，避免不必要的网络开销
- **错误处理**：完善的错误捕获和处理机制，确保应用稳定性
- **多模式支持**：支持本地文件获取和远程配置获取两种模式

### 认证模式获取
`useAuthModeFetch` 提供了完整的数据获取解决方案：

- **本地获取**：通过 `/public/siyuan-blog/` 目录获取静态资源
- **远程获取**：通过 `/api/endpoint` 端点获取远程配置
- **Siyuan SPA 检测**：自动检测运行环境，选择合适的获取方式
- **缓存机制**：支持客户端本地存储，提升二次访问速度

```mermaid
flowchart TD
Start(["数据获取请求"]) --> CheckEnv{"检查运行环境"}
CheckEnv --> |Siyuan SPA| SiyuanFetch["通过 Siyuan SPA 获取"]
CheckEnv --> |Node/Server| EndpointFetch["通过端点获取"]
SiyuanFetch --> BuildURL["构建获取URL"]
BuildURL --> FetchData["发起数据请求"]
EndpointFetch --> BuildEndpoint["构建端点请求"]
BuildEndpoint --> FetchData
FetchData --> CacheData["缓存到本地存储"]
CacheData --> Success["返回成功数据"]
FetchData --> HandleError["处理错误"]
HandleError --> ReturnDefault["返回默认数据"]
Success --> End(["结束"])
ReturnDefault --> End
```

**图表来源**
- [apps/app/composables/useAuthModeFetch.ts:15-324](file://apps/app/composables/useAuthModeFetch.ts#L15-L324)

### 静态页面加载优化
静态页面组件实现了智能的加载优化：

- **条件加载**：在非 Siyuan SPA 环境下预加载设置，在 Siyuan SPA 环境下延迟加载
- **骨架屏**：加载期间显示骨架屏，提升用户体验
- **错误降级**：加载失败时优雅降级，不影响整体功能

**章节来源**
- [apps/app/composables/useAuthModeFetch.ts:15-324](file://apps/app/composables/useAuthModeFetch.ts#L15-L324)
- [apps/app/components/static/HomePage.vue:23-42](file://apps/app/components/static/HomePage.vue#L23-L42)
- [apps/app/components/static/DetailPage.vue:10-21](file://apps/app/components/static/DetailPage.vue#L10-L21)

## AI 助手与 viewerCapabilities

### viewerCapabilities 组合式钩子
`useViewerCapabilities` 提供了查看器能力检测功能：

- **查看器类型检测**：基于运行时配置检测查看器类型（siyuan/node/vercel/cloudflare）
- **AI 助手支持检测**：检测查看器是否支持 AI 助手功能
- **构建时限制**：在构建时根据目标环境禁用特定功能

### AI 助手部署规则
根据开发文档，AI 助手功能遵循严格的部署规则：

- **服务器依赖**：AI 功能仅在具备服务器能力的查看器中启用
- **构建时禁用**：Siyuan 目标构建时会禁用 AI 助手功能
- **运行时验证**：通过 `viewerCapabilities.aiAssistant` 和 `post.aiAssistantEnabled` 双重验证

```mermaid
flowchart TD
Start(["AI 助手请求"]) --> CheckViewer["检查查看器能力"]
CheckViewer --> |支持| CheckPost["检查文章设置"]
CheckViewer --> |不支持| DisableAI["禁用 AI 助手"]
CheckPost --> |启用| EnableAI["启用 AI 助手"]
CheckPost --> |禁用| DisableAI
EnableAI --> HasContent{"是否有内容"}
HasContent --> |有内容| ShowAI["显示 AI 助手"]
HasContent --> |无内容| HideAI["隐藏 AI 助手"]
DisableAI --> End(["结束"])
ShowAI --> End
HideAI --> End
```

**图表来源**
- [apps/app/composables/useViewerCapabilities.ts:1-15](file://apps/app/composables/useViewerCapabilities.ts#L1-L15)
- [DEVELOPMENT.md:36-47](file://DEVELOPMENT.md#L36-L47)

### 构建脚本集成
构建脚本 `siyuan.sh` 集成了 AI 助手的构建时禁用逻辑：

- **配置切换**：自动切换到 Siyuan 构建配置
- **AI 禁用提示**：明确提示 AI 助手在该目标中被禁用
- **资源复制**：构建完成后自动复制资源文件

**章节来源**
- [apps/app/composables/useViewerCapabilities.ts:1-15](file://apps/app/composables/useViewerCapabilities.ts#L1-L15)
- [apps/app/composables/useSiyuanSPA.ts:1-16](file://apps/app/composables/useSiyuanSPA.ts#L1-L16)
- [DEVELOPMENT.md:36-47](file://DEVELOPMENT.md#L36-L47)
- [apps/app/script/siyuan.sh:29-37](file://apps/app/script/siyuan.sh#L29-L37)

## 依赖关系分析
- 分享页依赖设置存储与静态分享处理，二者均通过 Siyuan API 封装访问内核能力
- Web 应用首页根据分享类型判断决定加载静态或公开页面，并通过主题加载器完成样式渲染
- 设置页与主题加载器之间无直接耦合，但共享配置（主题名、版本、自定义 CSS）
- **新增** SPA 数据获取系统通过 `useAuthModeFetch` 统一管理数据获取逻辑
- **新增** viewerCapabilities 为所有页面提供能力检测和功能控制

```mermaid
graph LR
ShareVue["Share.vue"] --> SettingStore["useSettingStore.ts"]
ShareVue --> StaticShare["useStaticShare.ts"]
ShareVue --> SiyuanAPI["useSiyuanApi.ts"]
IndexVue["index.vue"] --> ShareType["useCommonShareType.ts"]
ShareType --> ShareEnum["ShareTypeEnum.ts"]
IndexVue --> ThemeMode["useClientThemeMode.ts"]
ThemeMode --> ThemeUtils["ThemeUtils.ts"]
IndexVue --> ViewerCaps["useViewerCapabilities.ts"]
ViewerCaps --> SiyuanSPA["useSiyuanSPA.ts"]
IndexVue --> AuthFetch["useAuthModeFetch.ts"]
AuthFetch --> HomePage["HomePage.vue"]
AuthFetch --> DetailPage["DetailPage.vue"]
```

**图表来源**
- [apps/siyuan/src/pages/Share.vue:30-34](file://apps/siyuan/src/pages/Share.vue#L30-L34)
- [apps/siyuan/src/stores/useSettingStore.ts:20-26](file://apps/siyuan/src/stores/useSettingStore.ts#L20-L26)
- [apps/siyuan/src/composables/useStaticShare.ts:17-20](file://apps/siyuan/src/composables/useStaticShare.ts#L17-L20)
- [apps/siyuan/src/composables/useSiyuanApi.ts:17-29](file://apps/siyuan/src/composables/useSiyuanApi.ts#L17-L29)
- [apps/app/pages/index.vue:17-22](file://apps/app/pages/index.vue#L17-L22)
- [apps/app/composables/useCommonShareType.ts:12-16](file://apps/app/composables/useCommonShareType.ts#L12-L16)
- [apps/app/enums/ShareTypeEnum.ts:13-25](file://apps/app/enums/ShareTypeEnum.ts#L13-L25)
- [apps/app/composables/useClientThemeMode.ts:23-27](file://apps/app/composables/useClientThemeMode.ts#L23-L27)
- [apps/app/utils/ThemeUtils.ts:18-24](file://apps/app/utils/ThemeUtils.ts#L18-L24)
- [apps/app/composables/useViewerCapabilities.ts:1-15](file://apps/app/composables/useViewerCapabilities.ts#L1-L15)
- [apps/app/composables/useSiyuanSPA.ts:1-16](file://apps/app/composables/useSiyuanSPA.ts#L1-L16)
- [apps/app/composables/useAuthModeFetch.ts:15-324](file://apps/app/composables/useAuthModeFetch.ts#L15-L324)
- [apps/app/components/static/HomePage.vue:1-75](file://apps/app/components/static/HomePage.vue#L1-L75)
- [apps/app/components/static/DetailPage.vue:1-21](file://apps/app/components/static/DetailPage.vue#L1-L21)

**章节来源**
- [apps/siyuan/src/pages/Share.vue:30-34](file://apps/siyuan/src/pages/Share.vue#L30-L34)
- [apps/app/pages/index.vue:17-22](file://apps/app/pages/index.vue#L17-L22)

## 性能考量
- 静态资源落盘：仅暴露必要字段并持久化 JSON，避免冗余数据传输与渲染压力
- 主题按需加载：默认主题与当前主题分两段加载，减少首屏样式计算
- 自定义 CSS 注入：按需注入，避免全局样式污染
- 过期时间校验：在前端进行范围校验，减少无效请求
- **新增** SPA 数据获取延迟加载：按需发起请求，减少不必要的网络开销
- **新增** 错误处理优化：完善的错误捕获和降级机制，提升应用稳定性
- **新增** 缓存机制：客户端本地存储提升二次访问速度

## 故障排查指南
- 无法访问分享链接
  - 检查"自定义域名"与"切换IP"设置，确保域名解析与端口正确
  - 若为外网访问，确认"网络伺服"已开启
- 无法看到最新内容
  - 若文档已更新，需重新"开启分享"以刷新静态资源
- 主页未生效
  - 确认主页 ID 对应的文档已开启分享
- 过期时间异常
  - 输入值需为非负整数且不超过 7 天（单位秒），0 表示永久
- **新增** SPA 数据获取问题
  - 检查运行环境是否为 Siyuan SPA 或 Node 服务器
  - 确认 `/public/siyuan-blog/` 目录存在且可访问
  - 查看浏览器控制台的错误日志
- **新增** AI 助手不可用
  - 确认当前查看器是否支持 AI 助手功能
  - 检查构建时是否为 Siyuan 目标（该目标会禁用 AI 助手）
  - 验证文章设置中的 AI 助手启用状态

**章节来源**
- [apps/siyuan/src/pages/Share.vue:139-202](file://apps/siyuan/src/pages/Share.vue#L139-L202)
- [apps/siyuan/src/pages/Setting/GeneralSetting.vue:95-118](file://apps/siyuan/src/pages/Setting/GeneralSetting.vue#L95-L118)
- [apps/siyuan/src/i18n/zh_CN.json:25-34](file://apps/siyuan/src/i18n/zh_CN.json#L25-L34)
- [apps/app/composables/useAuthModeFetch.ts:15-324](file://apps/app/composables/useAuthModeFetch.ts#L15-L324)
- [DEVELOPMENT.md:36-47](file://DEVELOPMENT.md#L36-L47)

## 结论
Siyuan Plugin Blog 通过"私有静态分享"模式，将 Siyuan 笔记安全地转化为可分享的静态网页。其核心在于：
- 分享页与静态分享处理的协同，确保内容与资源完整落盘
- 设置存储与设置页的解耦设计，便于扩展与维护
- 主题系统的动态加载与自定义 CSS 支持，满足多样化的视觉需求
- 分享类型与权限控制的统一策略，保障安全性与可用性
- **新增** SPA 数据获取系统的延迟加载和错误处理改进，提升了性能和稳定性
- **新增** viewerCapabilities 组合式钩子的引入，提供了灵活的能力检测和功能控制

## 附录
- 使用示例与最佳实践
  - 基础分享：在"分享页"开启"在线分享"，复制链接给他人
  - 自定义域名：在"基础设置"中填写完整地址（含端口），并在"切换IP"中选择对应主机
  - 主题选择：在"基础设置"中选择浅色/深色主题，建议与个人阅读习惯一致
  - 分享模板：在"高级设置"中配置分享文案模板，支持占位符替换
  - 过期控制：为敏感内容设置合理过期时间，到期后需重新开启分享
  - 一键清理：当需要批量回收分享时，使用"清除所有分享"功能
  - **新增** SPA 优化：利用延迟加载特性，在大量内容场景下提升加载性能
  - **新增** AI 助手：在支持的查看器环境中享受 AI 辅助功能，注意构建目标对功能的影响