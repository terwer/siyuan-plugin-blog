...
## 更新摘要
**所做更改**
- 新增AI功能配置章节，涵盖AI使用限制、服务协议、PostMeta显示选项等新配置项
- 更新配置模型，添加postMetaEnabled和aiSummaryEnabled字段
- 新增AI功能使用计数器和协议确认机制
- 更新设置页面以支持AI相关配置项
...
- AI功能配置：AI使用限制、服务协议、PostMeta显示选项等新配置项
- 最佳实践与常见问题解决方案
...
subgraph "AI功能"
AICONFIG["AI配置模型<br/>postMetaEnabled<br/>aiSummaryEnabled"]
AICOUNTER["AI使用计数器<br/>每日30次限制"]
AITERMS["AI服务协议<br/>使用须知"]
END
IDX --> TAB
TAB --> GEN
TAB --> ADV
...
- [apps/app/app.config.ts:59-62](file://apps/app/app.config.ts#L59-L62)
...
- **新增** AI功能配置：支持AI使用限制、服务协议确认、PostMeta显示控制等AI相关配置。
...
- **新增** AI功能字段：postMetaEnabled（文档元信息栏开关）、aiSummaryEnabled（AI助手按钮显示开关）。
...
**图表来源**
- [apps/app/app.config.ts:28-77](file://apps/app/app.config.ts#L28-L77)
...
- **新增** 文档元信息栏配置（postMetaEnabled）
  - 作用：控制文档页面顶部元信息栏的显示，包含阅读时间、发布日期等功能入口。
  - 建议：默认关闭以避免影响历史用户体验，按需启用。

- **新增** AI助手按钮配置（aiSummaryEnabled）
  - 作用：控制是否在元信息栏显示AI助手按钮，启用后用户可使用AI速读功能。
  - 建议：默认关闭，用户需同意服务协议后方可使用。
...
### AI使用限制与计数器
系统提供AI功能的使用限制机制，确保服务稳定运行：

- **每日使用限制**：默认每日30次使用限制，防止过度使用
- **本地存储计数**：使用localStorage记录当日使用次数，按天重置
- **计数器实现**：通过useAIUsage组合式函数管理使用状态

```mermaid
flowchart TD
Start(["AI功能调用"]) --> CheckLimit{"检查使用限制"}
CheckLimit --> |剩余次数>0| CheckTerms{"检查服务协议"}
CheckLimit --> |剩余次数=0| ShowLimit["显示使用限制提示"]
CheckTerms --> |已同意| Proceed["执行AI调用"]
CheckTerms --> |未同意| ShowTerms["显示服务协议"]
Proceed --> Consume["消耗一次使用次数"]
Consume --> UpdateCount["更新localStorage计数"]
ShowLimit --> End(["结束"])
ShowTerms --> End
UpdateCount --> End
```
...
**图表来源**
- [apps/app/composables/useAIUsage.ts:62-114](file://apps/app/composables/useAIUsage.ts#L62-L114)
- [apps/app/components/ai-assistant/AIPanel.vue:66-96](file://apps/app/components/ai-assistant/AIPanel.vue#L66-L96)

### AI服务协议与合规
系统强制用户同意AI服务协议，确保合规使用：

- **协议内容**：包含数据处理说明、结果免责声明、服务说明等
- **协议存储**：用户同意后将状态存储在localStorage中
- **协议检查**：每次AI功能调用前检查协议状态

协议核心条款：
1. **数据处理**：文档内容将发送至AI服务进行分析处理
2. **结果说明**：AI生成内容仅供参考，可能存在不准确之处
3. **服务说明**：默认使用内置AI代理服务，支持自定义API Key
4. **免责声明**：AI生成内容不代表作者或平台观点

### PostMeta显示选项
AI功能与文档元信息栏深度集成：

- **元信息栏控制**：通过postMetaEnabled控制元信息栏整体显示
- **AI按钮控制**：通过aiSummaryEnabled控制AI助手按钮显示
- **状态共享**：多个组件共享AI面板激活状态
- **条件渲染**：根据设置动态决定组件显示
...
PostMetaConfig --> PostMetaComponent
PostMetaConfig --> AIPanel
PostMetaComponent --> AIPanel
```

**图表来源**
- [apps/app/components/static/content/PostMeta.vue:13-23](file://apps/app/components/static/content/PostMeta.vue#L13-L23)
- [apps/app/components/static/content/Main.vue:32-34](file://apps/app/components/static/content/Main.vue#L32-L34)
- [apps/app/components/ai-assistant/AIPanel.vue:66-96](file://apps/app/components/ai-assistant/AIPanel.vue#L66-L96)
...
- [apps/app/components/static/content/PostMeta.vue:1-231](file://apps/app/components/static/content/PostMeta.vue#L1-L231)
- [apps/app/components/static/content/Main.vue:1-35](file://apps/app/components/static/content/Main.vue#L1-L35)
- [apps/app/components/ai-assistant/AIPanel.vue:61-113](file://apps/app/components/ai-assistant/AIPanel.vue#L61-L113)
...
IDX --> TAB
CFG --> AICONFIG["AI配置模型"]
AICONFIG --> AICOUNTER["AI使用计数器"]
AICONFIG --> AITERMS["AI服务协议"]
AICOUNTER --> USAGECONST["Constants.ts"]
AITERMS --> TERMSFILE["ai-terms.txt"]
```
...
- **新增** AI功能优化：使用localStorage进行AI使用计数，避免频繁网络请求；AI调用前进行协议检查，减少无效请求。
...
## 结论
设置管理系统通过清晰的分层设计与完善的存储适配，实现了配置的持久化、可扩展与易维护。结合国际化与日志体系，能够满足多场景下的配置管理需求。**新增的AI功能配置进一步增强了系统的智能化能力，通过使用限制、协议确认和灵活的显示控制，为用户提供安全、合规且易用的AI体验。** 建议在生产环境中关注缓存命中率与序列化性能，并持续完善配置校验与回滚机制。
...
- **新增** AI功能：postMetaEnabled（文档元信息栏开关）、aiSummaryEnabled（AI助手按钮开关）
...
### AI功能常量定义
- AI使用协议键：AI_SUMMARY_TERMS_KEY
- AI使用计数键：AI_USAGE_KEY  
- AI每日限制：AI_DAILY_LIMIT（默认30次）
- AI自定义配置键：AI_CUSTOM_CONFIG_KEY
