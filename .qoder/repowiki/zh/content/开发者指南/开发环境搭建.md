...
- [apps/app/script/dev-prod.sh](file://apps/app/script/dev-prod.sh)
...
## 更新摘要
**所做更改**
- 新增 devProd 开发脚本章节，介绍生产类似环境的开发测试方法
- 更新开发服务器启动章节，包含 devProd 与传统 dev 脚本的对比
- 新增生产环境模拟配置说明，包括环境变量和端口设置
- 更新故障排除指南，增加 devProd 相关问题的排查方法
...
**更新** 新增 devProd 开发脚本，该脚本允许在生产类似环境中进行开发测试，增强了开发工作流程的灵活性和准确性。
...
**更新** 新增 devProd 开发脚本，该脚本通过复制 Node 配置文件并使用生产环境的 Provider 设置，提供更接近真实生产环境的开发体验。
...
## 架构总览
下图展示了开发阶段的典型交互：开发者通过根脚本触发任务，Turbo 并行调度各子包任务；前端应用分别启动开发服务器，插件前端在 Siyuan 中加载；Node Provider 模式下，本地 Node 服务提供数据与渲染能力。新增的 devProd 脚本提供了生产环境模拟的开发选项。
...
Turbo --> AppDevProd["apps/app 生产环境模拟<br/>devProd"]
Turbo --> SiyuanDev["apps/siyuan 开发服务器<br/>Vite dev"]
AppDev --> Browser["浏览器访问<br/>http://localhost:3000"]
AppDevProd --> BrowserProd["浏览器访问<br/>http://localhost:4000"]
...
- [apps/app/script/dev-prod.sh:12-15](file://apps/app/script/dev-prod.sh#L12-L15)
...
### 开发服务器启动与热重载
- Web 应用（apps/app）
  - 传统开发模式：通过根脚本启动 Nuxt 开发服务器，支持 --host 参数对外网访问。
  - 热重载：Nuxt 内置热更新机制，修改源码后浏览器自动刷新。
  - 国际化与静态资源：Nuxt 配置了多语言与静态资源注入，开发时自动加载本地资源。
- 生产环境模拟（devProd）
  - 通过 apps/app/script/dev-prod.sh 启动，该脚本会：
    - 复制 nuxt.node.config.ts 为 nuxt.config.ts
    - 设置环境变量：NUXT_PUBLIC_DEFAULT_TYPE=node、NUXT_PUBLIC_PROVIDER_MODE=true、NUXT_PUBLIC_PROVIDER_URL=https://api.terwergreen.com
    - 使用端口 4000 启动开发服务器
    - 支持 --host 参数对外网访问
  - 优势：提供更接近真实生产环境的开发体验，包括生产环境的 Provider 配置和端口设置。
...
**更新** 新增 devProd 开发脚本的详细说明，包括其与传统开发模式的区别和优势。
...
- 生产环境模拟调试
  - devProd 脚本使用端口 4000，便于与传统开发模式区分，避免端口冲突。
  - 通过 https://api.terwergreen.com 提供的远程 Provider 服务进行开发测试。

**更新** 新增 devProd 调试工具的相关说明。
...
- 开发服务器验证
  ...
  - 启动 apps/app devProd 脚本后，访问 http://localhost:4000 验证生产环境模拟功能。
...
- 环境变量验证
  ...
  - devProd 脚本启动时的环境变量应正确应用到生产环境模拟模式。

**更新** 新增 devProd 环境验证方法，包括端口 4000 的访问验证和环境变量检查。
...
- [package.json:4-21](file://package.json#L4-L21)
- [turbo.json:3-14](file://turbo.json#L3-L14)
- [apps/app/package.json:5-12](file://apps/app/package.json#L5-L12)
...
- devProd 脚本使用独立端口（4000）避免与传统开发模式冲突，提高开发效率。

## 故障排除指南
- 端口冲突
  - apps/app 默认端口为 3000，devProd 脚本使用端口 4000；apps/siyuan 默认端口为 6808；如冲突，可在启动命令中添加 --port 参数或通过环境变量调整。
...
- Provider 模式连接失败
  ...
  - 确认 Node 服务已启动且端口开放。
  - 验证网络连接和防火墙设置。
  - 对于 devProd 脚本，确认 https://api.terwergreen.com 服务可用性。

**更新** 新增 devProd 相关问题的排查方法，包括端口冲突和远程服务连接问题。
...
## 结论
通过本指南，您可以在本地快速搭建并运行本项目的开发环境。建议优先使用 pnpm 与 Node 18+，遵循 monorepo 工作区配置，利用根脚本与 Turbo 实现高效开发与构建。新增的 devProd 开发脚本提供了生产环境模拟的开发选项，有助于在更接近真实生产环境的情况下进行开发测试。遇到问题时，可依据故障排除指南逐项排查，确保开发流程顺畅。

**更新** 新增 devProd 开发脚本的使用指导，帮助开发者更好地利用生产环境模拟功能。

## 附录
- 快速启动命令
  - 安装依赖：pnpm install
  - 启动 Web 应用（传统模式）：pnpm dev -F @terwer/share-pro-app -- --host
  - 启动 Web 应用（生产环境模拟）：pnpm devProd -F @terwer/share-pro-app
...
- 关键配置文件
  ...
  - apps/app/nuxt.node.config.ts：Node Provider 模式配置
  - apps/siyuan/vite.config.ts：Vite 开发配置与热重载
  - pnpm-workspace.yaml：工作区与仅构建依赖声明
  - turbo.json：任务编排与缓存策略
  - apps/app/script/dev.sh：传统开发脚本
  - apps/app/script/dev-prod.sh：生产环境模拟脚本
- 环境变量参考
...
**更新** 新增 devProd 脚本、相关配置文件和端口参考信息。