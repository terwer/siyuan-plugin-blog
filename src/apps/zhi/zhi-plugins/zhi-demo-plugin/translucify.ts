...
class Translucify {
  ...
  private _toleranceValue = DEFAULT_TOLERANCE_VALUE

  /**
   * @param {HTMLImageElement} image
   * @returns {boolean}
   */
  private isImageLoaded(image: any) {
    if (
      image.nodeType === 1 &&
   ...
   */
  private getCanvasAndCORSImage(image: any) {
    /*
   ...
   * @param {HTMLImageElement} image
   */
  private modifyImagePixels(image: any) {
    const that = this
    const created = this.getCanvasAndCORSImage(image)

    created.imageCORS.onload = function () {
      that.applyFloodFill(image, created.imageCORS, created.canvas)
    }
    created.imageCORS.src = image.src

    // Apply filter immediately if imageCORS is loaded from cache
    // and doesn't fire the load event.
    if (this.isImageLoaded(created.imageCORS)) {
      this.applyFloodFill(image, created.imageCORS, created.canvas)
    }
  }
   ...
   */
  private floodFill(x: number, y: number, context: any, tolerance: number) {
    var pixelStack = [[x, y]]
   ...
   */
  private applyFloodFill(originalImage: any, imageCORS: any, canvas: any) {
    var ctx = canvas.getContext("2d")
    ctx.drawImage(imageCORS, 0, 0)
    this.floodFill(0, 0, ctx, this._toleranceValue)
  ...
  private translucifyOneFloodFill(image: any) {
    if (this.isImageLoaded(image)) {
      this.modifyImagePixels(image)
    } else {
   ...
   * @param {number} [tolerance]
   */
  translucifyAll(queryResult: any, tolerance?: number) {
    if (typeof tolerance !== "undefined") {
      this._toleranceValue = tolerance
    } else {
      this._toleranceValue = DEFAULT_TOLERANCE_VALUE
    }

    if (queryResult instanceof HTMLImageElement) {
      // <img> passed in directly
      this.translucifyOneFloodFill(queryResult)
    } else if (queryResult instanceof NodeList) {
      // document.querySelectorAll support
      Array.prototype.slice
        .call(queryResult)
        .forEach(this.translucifyOneFloodFill)
    } else {
      // jQuery object support
      if (queryResult && queryResult.toArray) {
        queryResult.toArray().forEach(this.translucifyOneFloodFill)
      }
    }
  }
}
