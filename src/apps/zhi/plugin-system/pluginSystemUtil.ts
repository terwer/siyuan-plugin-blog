...
const logger = ZhiUtil.zhiSdk().getLogger()
...
const getPluginSystem = () => {
  return siyuanUtil.syWin().pluginSystem
}
const getPluginSystemVersion = () => {
  return siyuanUtil.syWin().pluginSystemVersion
}
...
export class HackPluginSystem {
  logger = ZhiUtil.zhiSdk().getLogger()
...
const initPluginSystem = async () => {
  ...
  try {
    ...
    return fetch(
      ...
      .then((res) => res.text())
      .then((sc) => {
        siyuanUtil.syWin().siyuanPluginScript = sc
        eval(sc)
      })
  }
